...
// Initialize the node, setup the NodeHandle for handling the communication with the ROS //system  
	...
	ros::ServiceClient client3 = nh.serviceClient<turtlesim_controller::Velocity>("/vel");
	...
	turtlesim_controller::Velocity service_vel;
	
	service_vel.request.min = 0.0;
	service_vel.request.max = 1.0;
	client3.waitForExistence();
	client3.call(service_vel);
	my_vel.linear.x = service_vel.response.x;
	my_vel.angular.z = service_vel.response.z;
